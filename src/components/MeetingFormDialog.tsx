...
export function MeetingFormDialog({ open, onOpenChange, defaultDate, defaultHour, meeting, onSuccess }: MeetingFormDialogProps) {
  ...
  const { data: profiles } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data.results ?? data) as Array<{ id: string; email: string }>;
    },
  });
  ...
  const onSubmit = async (values: MeetingFormValues) => {
    ...
    try {
      ...
      if (isEditMode && meeting) {
        await api.patch(`/meetings/${meeting.id}/`, payload);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn("max-h-[90vh] overflow-y-auto", showMap ? "max-w-4xl" : "max-w-lg")}
        onInteractOutside={(e) => {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <FormField control={form.control} name="type" render={({ field }) => (
              <FormItem>
                ...
                <div className="flex gap-2">
                  <Button type="button" variant={field.value === "offline" ? "default" : "outline"} className="flex-1" onClick={() => field.onChange("offline")}>
                    <Building className="mr-2 h-4 w-4" />Offline
                  </Button>
                  <Button type="button" variant={field.value === "online" ? "default" : "outline"} className="flex-1" onClick={() => field.onChange("online")}>
            ...
            {meetingType === "offline" && (
              <div className="space-y-4">
                {showMap && <div className="rounded-lg overflow-hidden"><MeetingLocationMap apiKey={apiKey} onLocationSelect={handleLocationSelect} /></div>}
                {isLoadingApiKey && <div className="flex items-center justify-center h-[320px] border rounded-lg"><Loader2 className="h-4 w-4 animate-spin" /><span className="ml-2 text-muted-foreground">Loading map...</span></div>}
                {apiKeyError && <div className="flex items-center justify-center min-h-[100px] border rounded-lg bg-muted/50"><p className="text-sm text-muted-foreground text-center px-4">Map unavailable. You can still enter the location manually.</p></div>}
              </div>
            )}

            <FormField control={form.control} name="location" render={({ field }) => (
              <FormItem>
                <FormLabel>{meetingType === "online" ? "Meeting Link" : "Location"}</FormLabel>
                <FormControl>
                  <div className="relative">
                    {meetingType === "online" ? <Video className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" /> : <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />}
                    <Input placeholder={meetingType === "online" ? "https://meet.google.com/..." : "Office address"} className="pl-10" {...field} />
                  </div>
                </FormControl>
                <FormMessage />
              </FormItem>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Input placeholder="https://maps.google.com/..." className="pl-10" {...field} />
                    </div>
                  </FormControl>
                  <FormMessage />
                </FormItem>
            ...
            <div className="grid grid-cols-2 gap-4">
              {(["start", "end"] as const).map((prefix) => (
                <FormField key={`${prefix}_date`} control={form.control} name={`${prefix}_date`} render={({ field }) => (
                  <FormItem className="flex flex-col">
                    <FormLabel>{prefix === "start" ? "Start" : "End"} Date</FormLabel>
                    <Popover>
            ...
            <div className="grid grid-cols-2 gap-4">
              {(["start_time", "end_time"] as const).map((name) => (
                <FormField key={name} control={form.control} name={name} render={({ field }) => (
                  <FormItem>
                    <FormLabel>{name === "start_time" ? "Start" : "End"} Time</FormLabel>
            ...
            <FormField control={form.control} name="notes" render={({ field }) => (
              <FormItem><FormLabel>Notes</FormLabel><FormControl><Textarea placeholder="Additional notes..." rows={3} {...field} /></FormControl><FormMessage /></FormItem>
            )} />
              ...
              {isEditMode ? (
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button type="button" variant="destructive" disabled={isDeleting}><Trash2 className="h-4 w-4 mr-2" />Delete</Button>
                  </AlertDialogTrigger>
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      <AlertDialogTitle>Delete Meeting</AlertDialogTitle>
                      <AlertDialogDescription>Are you sure you want to delete this meeting?</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
